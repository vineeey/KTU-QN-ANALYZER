...
{{
  ...
  "relationship": "identical|rewording|generalization|different_subtopic|completely_different"
    ...
    # ========== STRICT REPETITION CONFIRMATION (Phase 5) ==========

    def confirm_same_question(
        self,
        text1: str,
        text2: str,
    ) -> Tuple[bool, int]:
        """
        Determine whether two question texts ask the SAME thing.

        This is the authoritative LLM confirmation step for strict
        repetition detection.  It implements the Phase 5 contract:

        Definition of SAME:
          - Same core concept
          - Same expected answer structure
          - Same depth and scope
          - Minor wording differences allowed

        NOT same:
          - Different context, scope, or focus
          - Different question types about the same topic
            (e.g. "Advantages of Spiral" vs "Phases of Spiral")
          - Compare/contrast questions vs explain/describe questions

        Examples:
          SAME:
            "Explain phases of Spiral model"
            "Describe the phases of Spiral model"
          NOT SAME:
            "Advantages of Spiral model"
            "When is Spiral model used"
            "Compare Spiral and Waterfall"

        Returns:
            (is_same, confidence) where:
              - is_same: True only if LLM says SAME=YES with confidence >= 80
              - confidence: integer 0–100 from LLM response
        """
        prompt = self._build_confirmation_prompt(text1, text2)
        response, _ = self._call_llm(prompt)

        if not response:
            logger.warning(
                "LLM returned no response for confirm_same_question. "
                "Defaulting to NOT SAME."
            )
            return False, 0

        is_same, confidence, reason = self._parse_confirmation_response(response)
        logger.debug(
            "LLM confirmation: SAME=%s, confidence=%d, reason=%r",
            is_same, confidence, reason,
        )
        # ONLY accept as same if both conditions met
        return is_same and confidence >= 80, confidence

    def _build_confirmation_prompt(self, q1: str, q2: str) -> str:
        """
        Build the strict Phase 5 confirmation prompt.

        The LLM must respond ONLY in the exact 3-line format — no extra text.
        Strict: when in doubt, respond NO.
        """
        return f"""You are evaluating whether two academic exam questions ask about the SAME thing.
...
DEFINITION OF "SAME":
- Same core concept AND same expected answer structure AND same depth/scope.
- Minor wording differences are acceptable.

DEFINITION OF "NOT SAME":
- Different focus, context, or scope — even if same broad topic.
- "Advantages of X" vs "Phases of X" → NOT SAME.
- "When to use X" vs "How X works" → NOT SAME.
- "Compare X and Y" vs "Explain X" → NOT SAME.
...
When in doubt, respond: Same: NO"""

    def _parse_confirmation_response(
        self, response: str
    ) -> Tuple[bool, int, str]:
        """
        Parse the strict 3-line Phase 5 LLM response.

        Expected format:
            Same: YES
            Confidence: 85
            Reason: Both questions ask about the phases of the Spiral model.

        Returns:
            (is_same, confidence, reason)
            - is_same: True only if response says "Same: YES"
            - confidence: integer 0–100, defaults to 0 on parse failure
        ...
        # Same: YES or NO
        same_m = re.search(r"(?i)^same\s*:\s*(yes|no)", response, re.MULTILINE)
        if same_m:
            is_same = same_m.group(1).upper() == "YES"
        else:
            # Fallback: look for YES/NO anywhere near the word "same"
            if re.search(r"(?i)\bsame\b.*\byes\b", response):
                is_same = True
            elif re.search(r"(?i)\bsame\b.*\bno\b", response):
                is_same = False

        # Confidence: 0-100
        conf_m = re.search(r"(?i)confidence\s*:\s*(\d{1,3})", response)
        if conf_m:
            raw_conf = int(conf_m.group(1))
            # Values 0–1 range might come from old format — convert
            if raw_conf <= 1:
                raw_conf = int(raw_conf * 100)
            confidence = min(100, max(0, raw_conf))
        ...
        # Safety: if confidence > 100 or < 0, clamp
