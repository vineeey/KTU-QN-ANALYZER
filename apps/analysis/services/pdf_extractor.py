...
class PDFExtractor:
    ...
    - Fallback: OCR (PaddleOCR) for scanned PDFs only
    """
    ...
    def _load_libraries(self):
        ...
        try:
            ...
            self.Image = Image
            logger.info("✓ PaddleOCR loaded (OCR available)")
        except ImportError:
    ...
    def extract_text(self, pdf_path: str) -> str:
        ...
        if self.paddle_ocr and self.Image:
            try:
                text = self._extract_with_ocr(pdf_path)
                if text:
                    logger.info(f"✓ Extracted {len(text)} chars using PaddleOCR")
                    return text
            except Exception as e:
    ...
    def _clean_extracted_text(self, text: str) -> str:
        ...
        # Fix broken words ONLY at line boundaries (hyphenated splits)
        # e.g., "vulnerabil-\nity" -> "vulnerability"
        text = re.sub(r'(\w+)-\n(\w)', r'\1\2', text)
    ...
    def _extract_with_ocr(self, pdf_path: str) -> str:
        """Extract text using PaddleOCR (fallback for scanned PDFs)."""
        import numpy as np
        text_parts = []
        ...
        for page_num, page in enumerate(doc):
            ...
            # Run PaddleOCR
            image = self.Image.open(io.BytesIO(img_data)).convert("RGB")
