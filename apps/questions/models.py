...
class QuestionEmbeddingCache(models.Model):
    """
    Dedicated cache table for question sentence-transformer embeddings.

    Separating the vector from the Question row keeps the main table
    lean and makes cache-miss queries straightforward.

    The ``vector`` field stores the embedding as a JSON list of floats.
    For production at scale, consider pgvector.
    """
    ...
    # Embedding as JSON list of floats (e.g., 384 dims for all-MiniLM-L6-v2)
    ...
    def __str__(self) -> str:
        dim = len(self.vector) if self.vector else 0
        return f"Embedding({dim}d) for Q#{self.question_id}"
